...
impl Primitive<'_> {
    ...
    pub fn plerp<T: Terpable>(&self, value: T) -> T::Output {
        self.terp(value, TerpKind::LinearPerpectiveDivide)
    }

    /// `L`inear int`ERP`olation (see `TerpKind` or glsl "noperspective")
    pub fn lerp<T: Terpable>(&self, value: T) -> T::Output {
        self.terp(value, TerpKind::Linear)
    }

    /// flat interpolation (see `TerpKind` or glsl "flat")
    pub fn flat<T: Terpable>(&self, value: T) -> T::Output {
        self.terp(value, TerpKind::Flat)
    }
}
...
pub enum TerpKind {
    //TODO: get rid of this enum and use shame_graph version instead
        ...
        let any = shame_graph::Context::with(|ctx| {
            match &mut ctx.shader_mut().stage_interface {
                shame_graph::StageInterface::Vertex { outputs: f2v, .. } => {
                    let (mut out, loc) =
                        f2v.push_interpolated(kind, Tensor::new(S::SHAPE, D::DTYPE), None);
