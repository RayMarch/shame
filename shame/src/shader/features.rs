...
use crate::rec::{
    texture_combined_sampler::*, AnyDowncast, AsTen, DType, IsShapeScalarOrVec, Rec, Shape, Stage,
    Ten, WriteOnly,
};
...
pub fn record_render_shaders(mut f: impl FnMut(RenderShaderFeatures)) -> (String, String) {
    ...
    let mut record = |shader_kind| {
        let ctx =
            shame_graph::Context::with_thread_local_context_enabled(shader_kind, on_error, || {
...
impl<It: Iterator<Item = u32>> Group<It> {
    ...
    pub fn combine_sampler<Out: TexSampleType, In: TexCoordType>(
        &mut self,
    ) -> CombineSampler<Out, In> {
        let tcsampler = CombineSampler::new();
        let opaque_ty = CombineSampler::<Out, In>::opaque_ty();
        let index = self.next_index();
        self.bindings.push((
            index,
            shame_graph::Binding::Opaque(opaque_ty, tcsampler.any()),
        ));
        tcsampler
    }
    ...
    pub fn texture<Out: TexSampleType, In: TexCoordType>(&mut self) -> Texture<Out, In> {
        ...
        self.bindings.push((
            index,
            Binding::Opaque(Texture::<Out, In>::opaque_ty(), texture.any()),
        ));
        texture
    }
}
...
impl<It: Iterator<Item = u32>> VertexStreamBuilder<It> {
    ...
    pub fn attributes_detailed<T: Fields>(
        &mut self,
        ...
        let non_tensor_error = |ctx: &Context, ty, name| -> Any {
            let fields_of = T::parent_type_name()
                .map(|p| format!(" of `{p}`"))
                .unwrap_or_default();
...
impl<It: Iterator<Item = u32>> FragmentOutputsBuilder<It> {
    fn next_location(&mut self) -> u32 {
        next_index(
            &mut self.output_location_iterator,
            "rendertarget output location",
        )
    }
    ...
    /// if possible.
    pub fn color_with_ident<Ten: AsTen>(
        &mut self,
        ident: Option<String>,
    ) -> WriteOnly<Ten::S, Ten::D>
    where
        Ten::S: IsShapeScalarOrVec,
    {
