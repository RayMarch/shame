...
use crate::rec::sampler_texture::{Texture, Sampler, TexSampleType};
use crate::rec::{IsShapeScalarOrVec, texture_combined_sampler::*, AsTen, Ten, Shape, DType, WriteOnly, Stage, AnyDowncast, Rec};
use shame_graph::{Any, Context, LocationIter};
use crate::rec::{TexCoordType, float4, uint3, uint};
...
pub fn current_shader() -> shame_graph::ShaderKind {
    shame_graph::Context::try_with(|ctx| {
        ctx.shader_kind()
...
pub fn is_recording() -> bool {
    shame_graph::Context::try_with(|_| ()).is_some()
}
...
pub fn record_render_shaders(mut f: impl FnMut(RenderShaderFeatures)) -> (String, String){
    ...
    let mut record = |shader_kind| {
        let ctx = shame_graph::Context::with_thread_local_context_enabled(shader_kind, on_error, || {
            shame_graph::Context::with(|ctx| {
                ctx.record_shader_main(|| {
                    f(RenderShaderFeatures::new())
                });
            });
        });
    ...
    (record(shame_graph::ShaderKind::Vertex), record(shame_graph::ShaderKind::Fragment))
}
...
impl RenderIO<'_> {
    ...
    pub fn uniform<T: AsTen>(&self, _location: u32) -> Ten<T::S, T::D> {
        unimplemented!()
    }
}
...
impl<It: Iterator<Item=u32>> Drop for Group<It> {
    fn drop(&mut self) {
        Context::with(|ctx| {
            ...
            shader.side_effects.push_bind_group(self.group_index, shame_graph::BindGroup::new(bindings));
        })
    }
}
...
pub struct VertexStreamBuilder<It: Iterator<Item=u32>> {
    #[allow(unused)] used_attribute_locations: Vec<u32>, //for detection of overlap errors //TODO: check that overlaps are still detected despite this not being used
...
impl<It: Iterator<Item=u32>> Group<It> {
    ...
    fn next_index(&mut self) -> u32 {
        next_index(&mut self.binding_index_iterator, "binding index")
    }
    ...
    pub fn combine_sampler<Out: TexSampleType, In: TexCoordType>(&mut self) -> CombineSampler<Out, In> {
        ...
        let index = self.next_index();
        self.bindings.push((index, shame_graph::Binding::Opaque(opaque_ty, tcsampler.any())));
        tcsampler
    }
    ...
    pub fn sampler(&mut self) -> Sampler {
        ...
        let index = self.next_index();
        self.bindings.push((index, Binding::Opaque(Sampler::opaque_ty(), sampler.any())));
        sampler
    }
    ...
    pub fn texture<Out: TexSampleType, In: TexCoordType>(&mut self) -> Texture<Out, In> {
        ...
        let index = self.next_index();
        self.bindings.push((index, Binding::Opaque(Texture::<Out, In>::opaque_ty(), texture.any())));
        texture
    }
}
...
impl<It: Iterator<Item=u32>> VertexStreamBuilder<It> {
    ...
    pub fn attributes<T: Fields>(&mut self) -> T {
        self.attributes_detailed().0
    }
            ...
            match &mut shader.stage_interface {
                StageInterface::Vertex { inputs, .. } => {
                    let (any, range) = inputs.push_vertex_attribute_with_location_iter(
                        &mut loc_iter, ten, Some(name.to_string())
                    );
                    Some((any, ten, range))
                },
                _ => None,
    ...
    where Ten::S: IsShapeScalarOrVec { //TODO: replace with better writeonly type
        ...
        Context::with(|ctx| {
            ...
            let any = match &mut shader.stage_interface {
                shame_graph::StageInterface::Fragment { outputs, .. } => outputs.push_color_attachment(location, tensor, ident),
                _ => Any::not_available(),
            };
...
impl<'a> Raster<'a> {
    ...
    pub(crate) fn new() -> Self {
        ...
        match crate::current_shader() {
            Vertex   => Self {needs_to_be_used: true, phantom: PhantomData},
            Fragment => Self {needs_to_be_used: false, phantom: PhantomData},
...
/// ids
pub struct WorkGroupSetup<'a> {pub(crate) _phantom: PhantomData<&'a ()>}
...
pub struct Ids {pub(crate) _phantom: PhantomData<()>}

impl Ids {

    /// glsl: `gl_NumWorkGroups`
    pub fn total_work_groups_dispatched(&self) -> uint3 {
        Any::c_num_work_groups().downcast(Stage::Uniform)
    }

    /// glsl: `gl_WorkGroupID`
    pub fn work_group3(&self) -> uint3 {
        Any::c_work_group_id().downcast(Stage::Uniform)
    }

    /// glsl: `gl_LocalInvocationID`
    pub fn local3(&self) -> uint3 {
        Any::c_local_invocation_id().downcast(Stage::Uniform)
    }
    ...
    pub fn local(&self) -> uint {
        Any::c_local_invocation_index().downcast(Stage::Uniform)
    }
    ...
    pub fn work_group_size(&self) -> uint3 {
        Any::c_work_group_size().downcast(Stage::Uniform)
    }
}
