...
impl<Out: TexSampleType, In: TexCoordType> TryKeepIdent<'_, CombineSampler<Out, In>> {
    #[allow(missing_docs)]
    pub fn store_ident(&self, name: &str) { self.0.any().aka(name); }
}

impl<T: Rec> TryKeepIdent<'_, UnsafeAccess<T>> {
    #[allow(missing_docs)]
    pub fn store_ident(&self, name: &str) { self.0.aka(name); }
}

impl<T: Rec, const N: usize> TryKeepIdent<'_, Array<T, Size<N>>> {
    #[allow(missing_docs)]

    pub fn store_ident(&self, name: &str) { self.0.as_any().aka(name); }
}
