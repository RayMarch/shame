...
use shame_graph::{shorthands::new_array_enumerate, Access, Any};
use std::{marker::PhantomData, rc::Rc};

use super::{fields::Fields, narrow_stages_or_push_error, scal, DType, IntoRec};
...
impl Sizing for Unsized {
    const CONST_LEN: Option<usize> = None;
}
impl<const N: usize> Sizing for Size<N> {
    const CONST_LEN: Option<usize> = Some(N);
}
impl Sizing for RecordedSize {
    const CONST_LEN: Option<usize> = None;
}
...
impl<T: Rec, Len: Sizing> Array<T, Len> {
    ...
    pub unsafe fn at_mut<D: DType>(&mut self, i: impl AsTen<S = scal, D = D>) -> &mut T {
        ...
        debug_assert!(matches!(
            any.ty_via_thread_ctx().map(|x| x.access),
            None | Some(Access::LValue)
        ));
    ...
    pub unsafe fn at<D: IsDTypeInteger>(&self, i: impl AsTen<S = scal, D = D>) -> T {
        ...
        T::from_downcast(
            self.any.subscript(any).copy(),
            narrow_stages_or_push_error([self.stage, stage]),
        )
    }
...
impl<T: Rec, Len: Sizing> Fields for Array<T, Len>
where
    Self: Rec,
{
...
impl<T: IntoRec, const N: usize> IntoRec for [T; N] {
    ...
    fn stage(&self) -> Stage {
        self.iter().fold(Stage::Uniform, |acc, x| acc & x.stage())
    }
}

impl<T: Rec + Default + Copy, const N: usize> Default for Array<T, Size<N>>
where
    T: IntoRec<Rec = T>, //always the case
{
    fn default() -> Self {
