...
fn make_compute_pipeline_layout(
    shame: &shame::ComputePipelineInfo,
    device: &wgpu::Device,
) -> wgpu::PipelineLayout {
    //TODO: i think these layouts should already exist and be passed in here... not sure though
    let layouts: Vec<_> = shame
...
fn make_vertex_format(shame: &shame::tensor_type::Tensor) -> wgpu::VertexFormat {
    ...
    match dtype {
        ...
        DType::F32 => match shape {
            ...
            Shape::Mat(_, _) => {
                panic!("matrix vertex-attributes not supported by wgpu at the moment")
            }
...
fn make_primitive_state(shame: &shame::RenderPipelineInfo) -> wgpu::PrimitiveState {
    ...
    wgpu::PrimitiveState {
        topology,
        strip_index_format: is_strip.then(|| {
            match shame.index_dtype.expect("no index dtype recorded") {
                shame::IndexDType::U32 => wgpu::IndexFormat::Uint32,
                shame::IndexDType::U16 => wgpu::IndexFormat::Uint16,
            }
        }),
...
fn make_depth_stencil_state(shame: &shame::RenderPipelineInfo) -> Option<wgpu::DepthStencilState> {
    shame
        ...
        .map(|target| wgpu::DepthStencilState {
            format: match target {
            ...
            depth_compare: match shame
                .depth_test
                .expect("no information about depth test recorded")
            {
...
fn make_color_texture_format(
    ...
    match shame {
        ...
        SC::RGBA_Surface => known_surface_format
            .expect("surface format not provided, but RGBA_Surface requested by shader"),
    }
}
...
fn make_fragment_state<'a>(
    shame: &shame::RenderPipelineInfo,
    module: &'a wgpu::ShaderModule,
    scratch: &'a mut Vec<wgpu::ColorTargetState>,
    known_surface_format: &Option<wgpu::TextureFormat>,
...
pub fn make_compute_pipeline(
    shame: &shame::ComputePipelineRecording,
    device: &wgpu::Device,
