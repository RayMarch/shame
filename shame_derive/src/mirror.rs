...
use syn::PathSegment;
use syn::{
    punctuated::Punctuated, DataStruct, DeriveInput, FieldsNamed, Ident, Path, Result, Token,
};
        ...
        let name_ext = match (pre_star, post_star) {
            ...
            (Some(_), Some(_)) => {
                return Err(syn::Error::new(
                    args.span(),
                    "only one asterisk allowed either before or after the identifier",
                ))
            }
        };
    ...
    match path {
        ...
        Some(path) => Path {
            ...
            segments: Punctuated::from_iter(
                path.segments
                    .iter()
                    .chain(std::iter::once(&PathSegment::from(ident)))
                    .cloned(),
            ),
        },
    }
...
pub fn impl_for_struct(
    ...
    let device_trait =
        push_ident_onto_path(&args.module_path, Ident::new("Device", Span::call_site()));
    ...
    match kind {
        Kind::Host => {
            ...
            let host_fields = map_fields(fields, |span, ident, ty| {
                quote_spanned! {span =>
                    #ident: #ty
                }
            });

            let device_fields = map_fields(fields, |span, ident, ty| {
                quote_spanned! {span =>
