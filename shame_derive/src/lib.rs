...
fn generate_fields_tokens(input: syn::DeriveInput) -> Result<TokenStream2, Error> {
    match util::parse(&input) {
        util::DeriveData::Struct(input, struct_data, named_fields) => Ok(
            derive_fields::impl_for_struct(input, struct_data, named_fields),
        ),
        _ => Err(syn::Error::new(
            input.span(),
...
fn generate_mirror_tokens(
    ...
    match util::parse(&input) {
        util::DeriveData::Struct(input, struct_data, named_fields) => Ok(mirror::impl_for_struct(
            kind,
            args,
            input,
            struct_data,
