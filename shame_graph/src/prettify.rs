...
fn try_read_word<'a>(input: &'a str, patterns: &[&str]) -> Option<(&'a str, &'a str)> {
    try_read_str(input, patterns).and_then(|(head, tail)| {
        (!tail.starts_with(|x: char| x.is_alphanumeric() || x == '_'))
        .then(|| (head, tail))
    })
}
...
fn try_read_from_until<'a>(input: &'a str, [a, b]: [&str; 2]) -> Option<(&'a str, &'a str)> {
    try_read_str(input, &[a]).map(|(_head, tail)| {
        tail.find(b).map(|len| input.split_at(a.len() + len + b.len())).unwrap_or((input, ""))
    })
}
        ...
        let found = HIGHLIGHTS.iter().find_map(|h| match *h {
            ...
            Number => {
                code.starts_with(|x: char| x.is_ascii_digit()).then(|| {
                    let len = code.chars().take_while(|x| x.is_ascii_digit() || ['.', 'u', 'U'].contains(x))
    ...
    match hexcode {
        ...
        Some(h) => {
            if use_256_color_mode {
                ...
                .map(|n8| (n8 as f32 / (256.0 / 6.0)) as u8);
                w.write_fmt(format_args!("\x1B[38;5;{}m", 16 + b + (6 * (g + 6 * r)))).ok()
            } else {
                let [r, g, b] = hex_to_rgb8(h);
