...
use super::{try_deduce_builtin_fn, try_deduce_builtin_var, Access, Expr, ExprKind, Named, Tensor, Ty};
...
pub fn args_as_types(args: &[Key<Expr>]) -> SmallVec<[Ty; EST_ARGS]> {
    Context::with(|ctx| args.iter().map(|&arg| ctx.exprs()[arg].ty.clone()).collect())
}
...
pub fn try_deduce_field_select(field: &IdentSlot, args: &[Ty]) -> Result<Ty, Error> {
        ...
        [ty] => {
            match &ty.kind {
                TyKind::Struct(Struct(Named(fields, _))) => {
                    ...
                    let result = fields
                        .iter()
                        .find_map(|Named(ty, ident)| (ident == field).then(|| ty.clone().into_access(Access::LValue)));
    ...
    assert!(
        result.as_ref().map_or(true, |ty| { ty.access == Access::LValue }),
        ...
        Constructor::TextureCombinedSampler(ctor_dtype_dims) => {
            ...
            match args {
                [texture, sampler] => match (&texture.kind, &sampler.kind) {
                    (TyKind::Opaque(OpaqueTy::Texture(arg_dtype_dims)), TyKind::Opaque(OpaqueTy::Sampler)) => {
...
pub fn try_deduce_swizzle(kind: &super::Swizzle, args: &[Ty]) -> Result<Ty, Error> {
    ...
    let shape = Shape::from_vec_len(swizzle.len()).expect("invalid swizzle length occured in type deduction");
...
pub fn try_deduce_tensor_operator(kind: &super::Operator, args: &[Ty]) -> Result<Ty, Error> {
    ...
    let result_tensor = match (tens.as_slice(), kind) {
        ...
        (
            &[_l, _r],
            AddAssign | SubAssign | MulAssign | DivAssign | RemAssign | ShiftLAssign | ShiftRAssign | AndAssign
            | XorAssign | OrAssign,
        ) => {
