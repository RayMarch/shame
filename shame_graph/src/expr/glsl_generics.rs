use super::{DType, Shape, TexDimensionality, Ty};
use crate::expr::{OpaqueTy, ShadowSamplerKind, TexDtypeDimensionality, TyKind};
use DType::*;
...
impl HighLevelGlslGeneric {
    pub fn new(glsl: GlslGeneric) -> Self {
        use GlslGeneric::*;
        let shadow = |k: ShadowSamplerKind| Ty::new(TyKind::Opaque(OpaqueTy::ShadowSampler(k)));
    ...
    pub fn try_instantiate(&self, ty: &Ty) -> Option<GlslGenericInstantiation> {
        match (self, &ty.kind) {
            (Specific(x), _) => {
                (ty.eq_ignore_access(x)).then(|| GlslGenericInstantiation::specific)
            }

            (GenXType(x), TyKind::Tensor(ten)) =>
            //GLSL 4.60 spec: "[...] where the input arguments (and corresponding output) can be float, vec2, vec3, or vec4, genFType is used"
            {
            ...
            (VecNF32F64, TyKind::Tensor(ten)) => match ten.shape {
                Shape::Vec(n) if [F32, F64].contains(&ten.dtype) => {
                    Some(GlslGenericInstantiation::vecN(n))
                }
                _ => None,
            },

            (GVec4F32I32U32, TyKind::Tensor(ten)) => ([F32, I32, U32].contains(&ten.dtype)
                && matches!(ten.shape, Shape::Vec(4)))
            .then(|| GlslGenericInstantiation::g(ten.dtype)),

            (
                GSampler(x),
                TyKind::Opaque(OpaqueTy::TextureCombinedSampler(TexDtypeDimensionality(
                    dtype,
                    sampler_kind,
                ))),
            ) => (x == sampler_kind).then(|| GlslGenericInstantiation::g(*dtype)),
