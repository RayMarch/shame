...
impl HighLevelGlslGeneric {
    ...
    pub fn try_instantiate(&self, ty: &Ty) -> Option<GlslGenericInstantiation> {
        match (self, &ty.kind) {
            (Specific(x), _) => (ty.eq_ignore_access(x)).then(|| GlslGenericInstantiation::specific),
            ...
            (
                GSampler(x),
                TyKind::Opaque(OpaqueTy::TextureCombinedSampler(TexDtypeDimensionality(dtype, sampler_kind))),
