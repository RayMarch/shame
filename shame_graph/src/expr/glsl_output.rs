...
fn operator_to_glsl(ex: &State, op: &Operator, args: &[Key<Expr>]) -> String {
        ...
        match (has_binding, &sub_expr.kind) {
            (false, ExprKind::Operator(sub)) => {
                ...
                if sub.lhs_lvalue && is_left_of_sub {
                    assert!(
                        matches!(sub_expr.ident, Some(_)),
                        "left hand side expression of operator{} doesn't have a binding",
                        sub.glsl_str
                    );
                }
                ...
                match op.glsl_prec == sub.glsl_prec {
                    true => {
                        debug_assert!(
                            op.glsl_assoc == sub.glsl_assoc,
                            "same precedence must imply same associativity"
                        );
    ...
    match &args {
        [a] => {
            ...
            let a = wrap_if(
                binds_stronger_than(op, *a, is_left_of_sub),
                expr_key_to_glsl(ex, *a, false),
            );
...
fn swizzle_to_glsl(ex: &State, sw: &Swizzle, args: &[Key<Expr>]) -> String {
    ...
    let expr_str = match args {
        ...
        _ => panic!(
            "invalid amount of expression arguments ({}) for swizzle operation",
            args.len()
        ),
    };
...
impl FloatingPoint for f32 {
    fn get_fp_category(&self) -> std::num::FpCategory { self.classify() }
}
...
fn copy_expr_to_glsl(ex: &State, expr: &Expr, comment: &str) -> String {
    ...
    match expr.args.as_slice() {
        ...
        _ => panic!(
            "trying to generate glsl of a copy expression with {} arguments, only 1 argument allowed.",
            expr.args.len()
        ),
    }
}
    ...
    match &expr.kind {
        ...
        ExprKind::Constructor(x) => match x {
            ...
            Constructor::Struct(x) => {
                let args = expr.args.iter().map(|expr_key| expr_key_to_glsl(ex, *expr_key, false));

                let Struct(Named(fields, ident)) = x;
                let ident = ex.valid_ident(**ident);

                let field_idents = fields.iter().map(|Named(_, id)| ex.valid_ident(**id));
...
fn field_select_to_glsl(ex: &State, field: &IdentSlot, args: &[Key<Expr>]) -> String {
    match args {
        ...
        _ => panic!(
            "trying to generate glsl of a field selection with {} arguments, only 1 argument allowed.",
            args.len()
        ),
    }
}
...
fn ty_to_glsl(ex: &State, ty: &Ty) -> String {
    match &ty.kind {
        ...
        TyKind::Tensor(Tensor { dtype, shape }) => match shape {
            ...
            Shape::Mat(m, n) => {
                if let DType::U32 | DType::I32 | DType::Bool = dtype {
                    panic!(
                        "trying to generate glsl representation of an unsupported matrix type: {:?}",
                        ty
                    )
                }
                match m == n {
        ...
        TyKind::Struct(Struct(Named(_, ident))) => ex.valid_ident(**ident).to_string(),
        ...
        TyKind::Opaque(x) => match x {
            OpaqueTy::TextureCombinedSampler(TexDtypeDimensionality(dtype, kind)) => format!(
                "{}sampler{}",
                dtype_to_glsl_prefix(*dtype),
                tex_dimensionality_to_glsl_suffix(*kind)
            ),
            OpaqueTy::ShadowSampler(kind) => format!("sampler{}Shadow", shadow_sampler_kind_to_glsl_suffix(*kind)),
            OpaqueTy::Sampler => "sampler".to_string(),
            OpaqueTy::Texture(TexDtypeDimensionality(dtype, kind)) => format!(
                "{}texture{}",
                dtype_to_glsl_prefix(*dtype),
                tex_dimensionality_to_glsl_suffix(*kind)
            ),
            OpaqueTy::Image(TexDtypeDimensionality(dtype, kind)) => format!(
                "{}image{}",
                dtype_to_glsl_prefix(*dtype),
                tex_dimensionality_to_glsl_suffix(*kind)
            ),
...
fn stmt_to_glsl(ex: &State, stmt: &Stmt) -> String {
    match &stmt.kind {
        StmtKind::VariableDecl(Named(ty, ident)) => {
            format!("{} {};", ty_to_glsl(ex, ty), ex.valid_ident(ident.0))
        }
        ...
        LoopInit => {
            ...
            if let Some(first) = first {
                match &first.kind {
                    ...
                    kind => {
                        panic!("invalid statement of kind {kind} in loop increment")
                        //TODO: make error
                    }
                }
            }
...
fn block_key_to_glsl(ex: &State, block_key: Key<Block>) -> String { block_to_glsl(ex, &ex.ctx.blocks()[block_key]) }
...
fn item_to_glsl(ex: &State, item: &Item) -> String {
    match item {
        ...
        Item::StructDef(struct_) => struct_def_to_glsl(ex, struct_),
    }
}
...
fn packing_to_glsl(packing: Packing) -> &'static str {
    match packing {
...
fn struct_layout_to_glsl(ex: &State, struct_layout: &InterfaceBlock) -> Result<String, Error> {
    let result = struct_layout
        .0
        .iter()
        .map(|any| -> Result<String, Error> {
            let (ty, ident) = ty_ident_of_any(any, ex).ok_or(Error::NA {
                reason: "interface block member not found",
            })?;
...
fn binding_to_glsl(
    ex: &State,
    set_binding_index: (u32, u32),
    binding: &Binding,
) -> Result<(String, Option<Packing>), Error> {
    use Binding::*;
    ...
    let result = match binding {
        Opaque(opaque_ty, any) => {
            ...
            let (ty, ident) = ty_ident_of_any(any, ex).ok_or(Error::NA {
                reason: "interface member not found",
            })?;

            assert!(
                opaque_ty.eq_ignore_access(&ty),
                "opaque-type binding has changed its type since the interface recording"
            );
...
fn push_constant_str(ex: &State, sfx: &SideEffects) -> Result<String, Error> {
    match sfx.push_constant {
        Some(any) => {
            let (ty, ident) = ty_ident_of_any(&any, ex).ok_or(Error::NA {
                reason: "unable to obtain push constant's identifier and type",
            })?;
            let ident_string = ex.valid_ident(*ident);
            let ty = ty_to_glsl(ex, &ty);
            let indent = ex.with_deeper_indent(|| ex.indent_string());
    ...
    let groups_str = sfx
        ...
        .map(|(set_i, bind_group)| {
            bind_group
                .0
                .iter()
                .map(move |(bind_i, binding)| -> Result<String, Error> {
                    let (binding_string, packing) = binding_to_glsl(ex, (*set_i, *bind_i), binding)?;
                    let packing_string = packing
                        .map(|p| format!("{}, ", packing_to_glsl(p)))
...
fn varyings_to_glsl(ex: &State, varyings: &Varyings) -> Result<String, Error> {
    ...
    varyings
        ...
        .iter()
        .enumerate()
        .map(|(index, (interp, any))| -> Result<String, Error> {
            ...
            let (ty, ident) = ty_ident_of_any(any, ex).ok_or(Error::NA {
                reason: "interpolated variable not found",
            })?;
...
fn stage_interface_to_glsl(ex: &State, stage: &StageInterface) -> Result<String, Error> {
    let result = match stage {
        StageInterface::Vertex { inputs, outputs } => {
            let in_string = inputs
                .vertex_attributes
                .iter()
                .map(|(locs, any)| -> Result<String, Error> {
                    let (ty, ident) = ty_ident_of_any(any, ex).ok_or(Error::NAInShaderKind {
                        expected: ShaderKind::Vertex,
                        found: ex.ctx.shader_kind,
                    })?;
                    let ty = ty_to_glsl(ex, &ty);
                    let ident_string = ex.valid_ident(*ident);
...
fn ident_satisfies_glsl_constraints(s: &str) -> bool {
    !(s.starts_with("gl_") || s.starts_with("__") || GLSL_WORDS_THAT_CANNOT_BE_IDENTIFIERS.contains(&s))
}
...
fn single_valid_ident_for_slot(slot: &Option<String>) -> String {
    match slot {
        Some(string) if is_ident(string) => match ident_satisfies_glsl_constraints(string) {
            true => string.clone(),
...
fn valid_idents_for_pool(idents: &Vec<Option<String>>) -> Vec<String> {
    ...
    debug_assert!(
        valids.iter().all_unique(),
        "identifiers contain duplicates after deduplication: {:?}",
        valids
    );
    debug_assert!(
        valids
            .iter()
            .all(|s| is_ident(s) && ident_satisfies_glsl_constraints(s)),
        "valid identifiers list contains identifiers that aren't valid: {:?}",
        valids
    );
    valids
}
...
impl GlslExportState<'_> {
    ...
    fn valid_ident(&self, key: Key<Option<String>>) -> &str { &self.valid_idents[key.index] }
...
impl Context {
    pub fn generate_glsl(&self) -> Result<String, Error> {
        ...
        if !side_effects.trim().is_empty() {
            elements.push(side_effects)
        }
