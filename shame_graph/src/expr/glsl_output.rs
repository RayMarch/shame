...
fn operator_to_glsl(ex: &State, op: &Operator, args: &[Key<Expr>]) -> String {
    ...
    match &args {
        ...
        [a, b] => {
            let a = wrap_if(
                binds_stronger_than(op, *a, false),
                expr_key_to_glsl(ex, *a, false),
            );
            let b = wrap_if(
                binds_stronger_than(op, *b, true),
                expr_key_to_glsl(ex, *b, false),
            );
        ...
        [a, b, c] => {
            let a = wrap_if(
                ex.ctx.exprs()[*a].ident.is_none(),
                expr_key_to_glsl(ex, *a, false),
            );
            let b = wrap_if(
                ex.ctx.exprs()[*b].ident.is_none(),
                expr_key_to_glsl(ex, *b, false),
            );
            let c = wrap_if(
                ex.ctx.exprs()[*c].ident.is_none(),
                expr_key_to_glsl(ex, *c, false),
            );
            format!("{a} {op_str} {b} : {c}")
        }
        _ => panic!(
            "unsupported glsl operator {:?} with {} arguments",
            op,
            args.len()
        ),
    }
...
fn floating_point_to_glsl<T: FloatingPoint>(t: &T) -> String {
    ...
    match t.get_fp_category() {
        ...
        cat => panic!(
            "cannot convert floating point number of category '{:?}' to glsl",
            cat
        ),
    }
}
...
fn expr_kind_to_glsl(ex: &State, expr: &Expr) -> String {
    match &expr.kind {
        ExprKind::GlobalInterface(_) => {
            panic!("global interface expr without identifier is invalid")
        }
...
fn flow_to_glsl(ex: &State, flow: &Flow) -> String {
    ...
    match flow {
        Flow::IfThen { cond, then } => format!("if ({}) {}", exp(cond), blk(then)),
        ...
        Flow::While { cond, body } => format!("if ({}) {}", exp(cond), blk(body)),
    }
}
...
fn ty_to_glsl(ex: &State, ty: &Ty) -> String {
    match &ty.kind {
        ...
        TyKind::Callable(x) => panic!(
            "trying to generate glsl representation of a callable type: {:?}",
            x
        ),
        ...
        .flat_map(|(set_i, bind_group)| {
            bind_group
                ...
                .map(move |(bind_i, binding)| -> Result<String, Error> {
                    let (binding_string, packing) =
                        binding_to_glsl(ex, (*set_i, *bind_i), binding)?;
