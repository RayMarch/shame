...
impl Tensor {
    pub const fn new(shape: Shape, dtype: DType) -> Self { Self { shape, dtype } }

    pub fn into_ty(&self) -> Ty { (*self).into() }

    pub fn byte_size(&self) -> usize { self.shape.comps_total() * self.dtype.byte_size() }
...
impl Shape {
    ...
    pub fn col_count(self) -> usize { self.dims().0 }

    pub fn row_count(self) -> usize { self.dims().1 }
    ...
    pub fn len(&self) -> usize { self.comps_total() }
