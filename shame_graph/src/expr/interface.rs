...
use crate::{
    any::Any,
    common::{ranges_overlap, IteratorExt},
    context::Context,
    error::Error,
};
...
impl InterfaceBlock {
    pub fn new(members: Vec<Any>) -> Self { Self(members) }
}
...
impl BindGroup {
    pub fn new(bindings: Vec<(Loc, Binding)>) -> Self { Self(bindings) }
}

impl BindGroup {
    pub fn bindings(&self) -> &[(Loc, Binding)] { &self.0 }
}
...
impl SideEffects {
    pub fn bind_groups(&self) -> &[(Loc, BindGroup)] { &self.bind_groups }
}
...
impl SideEffects {
    ...
    pub fn set_push_constant(&mut self, tensor: Tensor, ident: Option<String>) -> Any {
        ...
        assert!(
            self.push_constant.is_none(),
            "only one push constant can be defined per shader"
        );
...
impl PrimitiveAssembly {
    pub fn push_vertex_attribute(&mut self, range: Range<Loc>, tensor: Tensor, ident: Option<String>) -> Any {
        assert!(
            range.len() == tensor.shape.col_count(),
            "range {:?} width does not match the amount of columns in {:?}",
            range,
            tensor.shape
        );
    ...
    pub fn push_vertex_attribute_with_location_iter<T: Iterator<Item = u32>>(
        &mut self,
        loc_iter: &mut LocationIter<T>,
        tensor: Tensor,
        ident: Option<String>,
    ) -> (Any, Range<u32>) {
        let loc_range = loc_iter
            .next(tensor.shape.col_count() as u32)
...
impl Framebuffer {
    pub fn push_color_attachment(&mut self, location: Loc, tensor: Tensor, ident: Option<String>) -> Any {
        assert!(
            tensor.shape.is_scalar_or_vec(),
            "color attachment tensor types may only be scalar or vec"
        );
...
impl Varyings {
    pub fn new(in_out: InOut) -> Self { Self(in_out, Default::default()) }
    ...
    pub fn validate(&self, name_in_errors: &str) -> Result<(), Error> {
        ...
        Context::with(|ctx| {
            self.1.iter().try_for_each(|(terp, any)| {
                match any.ty(&ctx.exprs()) {
                    Some(ty) => match ty.kind {
                        TyKind::Tensor(Tensor { dtype, shape: _ }) => match dtype {
                            Bool => Err(Error::TypeError(format!(
                                "Interpolating `{ty}` is invalid. {name_in_errors} cannot be of any bool type"
                            ))),
                            F64 | I32 | U32 if terp != &Interpolation::Flat => Err(Error::TypeError(format!(
                                "{name_in_errors} {ty} must use Flat interpolation, but uses: {:?}",
                                terp
                            ))),
                            _ => Ok(()),
                        },
        ...
        match self {
            StageInterface::Vertex { inputs, outputs } => {
                if let Some((a, b)) = inputs
                    .vertex_attributes
                    .iter()
                    .find_pair(|(a, _), (b, _)| ranges_overlap(a, b).then(|| (a, b)))
                {
            ...
            StageInterface::Fragment { inputs, outputs } => {
                ...
                if let Some(loc) = outputs
                    .color_attachments
                    .iter()
                    .find_pair(|(a, _), (b, _)| (a == b).then(|| a))
                {
...
impl SideEffects {
    pub fn validate(&self) -> Result<(), Error> {
        if let Some(overlap) = self.bind_groups.iter().find_pair(|(a, _), (b, _)| (a == b).then(|| a)) {
            Err(Error::OverlappingBindGroupIndex {
                duplicate_index: *overlap,
            })?;
        }

        for (group_i, group) in &self.bind_groups {
            if let Some(overlap) = group.0.iter().find_pair(|(a, _), (b, _)| (a == b).then(|| a)) {
            ...
            for (_, binding) in &group.0 {
                match binding {
                    Binding::Opaque(opaque_ty, any) => match opaque_ty {
                        OpaqueTy::TextureCombinedSampler(_) | OpaqueTy::ShadowSampler(_) => {
                    ...
                    Binding::Storage(interface_block) | Binding::StorageMut(interface_block) => {
                        match interface_block.split_last() {
                            Some((_last, rest)) => {
                                let result = Context::with(|ctx| {
                                    let exprs = ctx.exprs();
                                    let rest_all_sized = rest
                                        .iter()
                                        .all(|any| any.ty(&exprs).map(|ty| ty.is_sized()).unwrap_or(true));
                    ...
                    Binding::UniformBlock(interface_block) => {
                        let all_sized = interface_block
                            .iter()
                            .all(|x| x.ty_via_thread_ctx().map(|ty| ty.is_sized()).unwrap_or(true));
                        if !all_sized {
                            return Err(Error::TypeError(
                                "uniform block cannot contain unsized types".to_string(),
                            ));
                        };
                    }
                    _ => (),
...
pub trait Bindable {
    //each of a derive(BindGroup) type's members
    type RecordingType;
